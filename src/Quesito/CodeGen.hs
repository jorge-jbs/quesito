...
loadIfSized :: L.MonadIRBuilder m => L.Operand -> LLTT.Type -> m L.Operand
loadIfSized op ty
  | isSized ty = do
      let ty' = typeGen ty
...
typeGen :: LLTT.Type -> L.Type
typeGen ty@(LLTT.Pi _ _ _) =
  let (args, ret) = LLTT.flattenPi ty
  in L.FunctionType (typeGen ret) (map typeGen args) False
